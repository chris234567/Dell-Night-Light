$store = ( Get-Content $PSScriptRoot/store.json -Raw ) | ConvertFrom-Json

if (-not $store.is_day) {
    $brightness = $store.day.brightness
} else {
    $brightness = $store.night.brightness
}

Set-Location $store.display_manager_path
./ddm.exe /SetBrightnessLevel $brightness

$store.is_day = -not $store.is_day
$store | ConvertTo-Json | Out-File $PSScriptRoot/store.json
